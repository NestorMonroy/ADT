...
**Importancia:** 🔴 CRÍTICA  
**Omitir este paso:** ❌ Causa omisiones de contenido
...
.. code-block:: text
   ...
   ☐ 1. Leer archivo principal COMPLETO (no solo inicio)
      ...
      ❌ NO hacer: Leer solo primeras 50 líneas
      ✅ SÍ hacer: Leer de línea 1 a línea N
...
.. code-block:: text
   ...
   ✅ CORRECTO (Sección 12 de arc42):
   
   ☑ 1. Leído section-12.md completo: 44 líneas
   ☑ 2. Componentes identificados:
      ☑ Título: "12. Glossary"
      ☑ Content (líneas 12-15)
      ☑ Motivation (líneas 17-21)
      ☑ Form (líneas 23-30)
      ☑ Plantilla vacía (línea 37)
      ☑ Further info (líneas 40-43)
   
   ☑ 3. Archivos asociados:
      ☑ Tips: 6 (t-12-1 a t-12-6)
      ☑ Ejemplos: 1 (glossary-htmlsc)
      Total: 8 archivos
   
   ☑ 4. Inventario creado: 8 archivos listados
   ☑ 5. Complejidad: Baja (44 líneas)
       Enriquecimiento esperado: +200% a +300%
   
   Resultado: 0 omisiones, 8/8 archivos traducidos ✅

**Contra-ejemplo - Sección 07 (versión inicial):**

.. code-block:: text

   ❌ INCORRECTO (Sección 07 inicial):
   ...
   Resultado: 6 omisiones, 8/14 archivos ❌
   
   Corrección: Aplicar PASO 0 correctamente
   Resultado final: 0 omisiones, 14/14 archivos ✅
...
.. code-block:: bash
   ...
   grep -q "Título" "$TRADUCIDO" && echo "✅ Título" || echo "❌ Título OMITIDO"
   grep -q "Content" "$TRADUCIDO" && echo "✅ Content" || echo "❌ Content OMITIDO"
   grep -q "Motivation" "$TRADUCIDO" && echo "✅ Motivation" || echo "❌ Motivation OMITIDO"
   grep -q "Form" "$TRADUCIDO" && echo "✅ Form" || echo "❌ Form OMITIDO"
   ...
   if [ $TRADUCIDOS -eq $ESPERADOS ]; then
       ...
       echo "   ❌ FALTAN $((ESPERADOS - TRADUCIDOS)) ARCHIVOS"
   fi
...
.. code-block:: text
   ...
   ☐ 1. Ejecutar script de verificación
      ...
      Resultado: _____ / _____ elementos ✅
   ...
   ☐ 2. Verificación visual contra original
      ...
      - Marcar cada párrafo verificado con ✅
...
.. code-block:: markdown
   ...
   **Sección:** XX - Nombre
   **Workflow:** vX.Y.Z
   **Estado:** ✅ COMPLETADA 100% (N/N archivos)
   ...
   **Tamaño total:** XX KB (N archivos)
   ...
   ✅ Checklist completado (N/N elementos)
   ✅ Compilación exitosa

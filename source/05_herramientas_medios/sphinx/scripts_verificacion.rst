...
.. code-block:: bash
   ...
   if [ ! -f "$ORIGINAL" ]; then
       echo -e "${RED}❌ Error: Archivo original no existe: $ORIGINAL${NC}"
       exit 1
   fi
   
   if [ ! -f "$TRADUCIDO" ]; then
       echo -e "${RED}❌ Error: Archivo traducido no existe: $TRADUCIDO${NC}"
       exit 1
   fi
   ...
   if [ $ORIG_LINES -gt 0 ]; then
       ...
       if [ $ENRIQUECIMIENTO -ge $MIN_ENRIQ ] && [ $ENRIQUECIMIENTO -le $MAX_ENRIQ ]; then
           echo -e "   ${GREEN}✅ Enriquecimiento apropiado${NC}"
           SCORE_TOTAL=$((SCORE_TOTAL + 20))
       elif [ $ENRIQUECIMIENTO -lt $MIN_ENRIQ ]; then
           echo -e "   ${YELLOW}⚠️  Enriquecimiento bajo (agregar más contenido)${NC}"
           SCORE_TOTAL=$((SCORE_TOTAL + 10))
       else
           echo -e "   ${YELLOW}⚠️  Enriquecimiento alto (verificar si es apropiado)${NC}"
   ...
   if [ $ORIG_SECTIONS -eq $TRAD_SECTIONS ] || [ $TRAD_SECTIONS -ge $ORIG_SECTIONS ]; then
       echo -e "   ${GREEN}✅ Secciones OK${NC}"
       SCORE_TOTAL=$((SCORE_TOTAL + 15))
   else
       echo -e "   ${RED}❌ Faltan secciones${NC}"
   fi
   ...
   if [ $TRAD_LISTS -ge $ORIG_LISTS ]; then
       echo -e "   ${GREEN}✅ Listas OK${NC}"
       SCORE_TOTAL=$((SCORE_TOTAL + 10))
   else
       echo -e "   ${YELLOW}⚠️  Menos items en traducción${NC}"
   ...
   if [ $TRAD_CODE -ge $ORIG_CODE ]; then
       echo -e "   ${GREEN}✅ Código preservado${NC}"
       SCORE_TOTAL=$((SCORE_TOTAL + 10))
   else
       echo -e "   ${RED}❌ Faltan bloques de código${NC}"
   fi
   CHECKS_TOTAL=$((CHECKS_TOTAL + 10))
   ...
   if [ -f "$SPHINX_DIR/conf.py" ]; then
       ...
       if [ $COMPILE_EXIT -eq 0 ] && [ $ERRORS -eq 0 ]; then
           ...
           echo -e "   ${YELLOW}⚠️  Compilación con warnings${NC}"
           SCORE_TOTAL=$((SCORE_TOTAL + 20))
       else
       ...
       echo -e "   ${YELLOW}⚠️  No se encontró conf.py (saltando check)${NC}"
       SCORE_TOTAL=$((SCORE_TOTAL + 25))
   fi
   ...
   echo "🔗 Referencias encontradas: $TRAD_REFS"
   
   if [ $TRAD_REFS -gt 0 ]; then
       echo -e "   ${GREEN}✅ Referencias presentes${NC}"
       SCORE_TOTAL=$((SCORE_TOTAL + 10))
   else
       echo -e "   ${YELLOW}⚠️  Sin referencias (verificar si son necesarias)${NC}"
       SCORE_TOTAL=$((SCORE_TOTAL + 5))
   fi
   CHECKS_TOTAL=$((CHECKS_TOTAL + 10))
   ...
   for TERMINO in "${TERMINOS_CONSERVAR[@]}"; do
       ...
       if [ $COUNT -gt 0 ]; then
           ...
           if [ $COUNT -eq $COUNT_TRAD ]; then
               echo -e "   ${GREEN}✅ '$TERMINO' conservado correctamente${NC}"
           fi
       fi
   done
...
.. code-block:: text
   ...
   📋 Tipo de documento: B (20-50 líneas)
   ...
   for ORIG in "$ORIGINAL_DIR"/*.md; do
       ...
       if [ -f "$TRAD" ]; then
           ...
           if [ $EXIT_CODE -eq 0 ]; then
               echo "✅ $NOMBRE - Aprobado"
               APROBADOS=$((APROBADOS + 1))
           else
               echo "❌ $NOMBRE - Necesita revisión"
...
.. code-block:: bash
   ...
   if [ "$resp1" = "s" ] && [ "$resp2" = "s" ] && [ "$resp3" = "s" ] && [ "$resp4" = "s" ]; then
       echo "✅ PASO 0 COMPLETO - Listo para traducir"
       exit 0
   else
       echo "❌ PASO 0 INCOMPLETO - NO comenzar traducción todavía"
       echo ""

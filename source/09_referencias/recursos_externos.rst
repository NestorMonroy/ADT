.. _recursos_externos:




Recursos Externos
=================

Enlaces a documentación oficial, tutoriales y herramientas externas útiles.

.. contents:: Contenido
   :depth: 2
   :local:




Documentación Oficial
=====================

Sphinx
======

**Sitio oficial:**
   https://www.sphinx-doc.org/

**Guías principales:**
   - Getting Started: https://www.sphinx-doc.org/en/master/usage/quickstart.html
   - RST Primer: https://www.sphinx-doc.org/en/master/usage/restructuredtext/basics.html
   - Configuración: https://www.sphinx-doc.org/en/master/usage/configuration.html

**Extensiones:**
   - Lista oficial: https://www.sphinx-doc.org/en/master/usage/extensions/index.html
   - Terceros: https://github.com/yoloseem/awesome-sphinxdoc

reStructuredText
================

**Especificación:**
   https://docutils.sourceforge.io/rst.html

**Quick Reference:**
   https://docutils.sourceforge.io/docs/user/rst/quickref.html

**Cheat Sheet:**
   https://github.com/ralsina/rst-cheatsheet

Pandoc
======

**Sitio oficial:**
   https://pandoc.org/

**Manual completo:**
   https://pandoc.org/MANUAL.html

**Demos:**
   https://pandoc.org/demos.html

Git
===

**Documentación:**
   https://git-scm.com/doc

**Pro Git Book:**
   https://git-scm.com/book/en/v2

**Tutoriales interactivos:**
   https://learngitbranching.js.org/




Tutoriales y Guías
==================

Sphinx
======

**Real Python - Sphinx Tutorial:**
   https://realpython.com/python-sphinx-documentation/

**Read the Docs - Tutorial:**
   https://docs.readthedocs.io/en/stable/tutorial/

**Sphinx Book Theme:**
   https://sphinx-book-theme.readthedocs.io/

reStructuredText
================

**Sphinx Tutorial:**
   https://www.sphinx-doc.org/en/master/usage/restructuredtext/index.html

**Docutils Tutorial:**
   https://docutils.sourceforge.io/docs/user/rst/quickstart.html

LaTeX
=====

**LaTeX Wikibook:**
   https://en.wikibooks.org/wiki/LaTeX

**Overleaf Learn:**
   https://www.overleaf.com/learn

**CTAN (Comprehensive TeX Archive):**
   https://www.ctan.org/

Markdown
========

**Markdown Guide:**
   https://www.markdownguide.org/

**CommonMark Spec:**
   https://commonmark.org/

**GitHub Flavored Markdown:**
   https://github.github.com/gfm/




Herramientas Online
===================

Editores y Previsualización
===========================

**RST Online Editor:**
   - LiveSphinx: https://livesphinx.herokuapp.com/
   - RST Editor: https://rst.ninjs.org/

**Markdown Editors:**
   - StackEdit: https://stackedit.io/
   - Dillinger: https://dillinger.io/

**LaTeX Editors:**
   - Overleaf: https://www.overleaf.com/
   - CoCalc: https://cocalc.com/

Generadores
===========

**Tabla de Contenidos:**
   - DocToc: https://github.com/thlorenz/doctoc

**Tablas:**
   - Tables Generator: https://www.tablesgenerator.com/
   - ASCII Table Generator: https://ozh.github.io/ascii-tables/

**Diagramas:**
   - PlantUML: https://www.plantuml.com/plantuml/
   - Mermaid Live: https://mermaid.live/
   - Draw.io: https://app.diagrams.net/

Conversores
===========

**Pandoc Online:**
   https://pandoc.org/try/

**LaTeX to HTML:**
   https://www.latex4technics.com/

**Markdown to HTML:**
   https://markdowntohtml.com/

Validadores
===========

**RST Lint:**
   https://github.com/twolfson/restructuredtext-lint

**Markdown Lint:**
   https://github.com/markdownlint/markdownlint

**Regex Tester:**
   https://regex101.com/




Temas y Plantillas
==================

Temas Sphinx
============

**Read the Docs Theme:**
   https://sphinx-rtd-theme.readthedocs.io/

**Book Theme:**
   https://sphinx-book-theme.readthedocs.io/

**Furo:**
   https://pradyunsg.me/furo/

**PyData Theme:**
   https://pydata-sphinx-theme.readthedocs.io/

**Material Theme:**
   https://bashtage.github.io/sphinx-material/

Plantillas de Proyecto
======================

**Sphinx Quickstart Templates:**
   https://github.com/readthedocs/template

**Cookiecutter Sphinx:**
   https://github.com/audreyfeldroy/cookiecutter-pypackage

**Sphinx Starter:**
   https://github.com/sphinx-templates/sphinx-quickstart-templates




Extensiones Útiles
==================

Sphinx Extensions
=================

**Myst Parser (Markdown):**
   https://myst-parser.readthedocs.io/

**Sphinx-Tabs:**
   https://sphinx-tabs.readthedocs.io/

**Sphinx-Copybutton:**
   https://sphinx-copybutton.readthedocs.io/

**Sphinx-Autobuild:**
   https://github.com/executablebooks/sphinx-autobuild

**Sphinx-Design:**
   https://sphinx-design.readthedocs.io/

Diagramas y Visualización
=========================

**Sphinx-PlantUML:**
   https://github.com/sphinx-contrib/plantuml

**Sphinx-Mermaid:**
   https://github.com/mgaitan/sphinxcontrib-mermaid

**Graphviz:**
   https://www.sphinx-doc.org/en/master/usage/extensions/graphviz.html

Multimedia
==========

**Sphinx-Inline-Tabs:**
   https://sphinx-inline-tabs.readthedocs.io/

**Sphinx-Gallery:**
   https://sphinx-gallery.github.io/

**Sphinx-Panels:**
   https://sphinx-panels.readthedocs.io/




Hosting y Despliegue
====================

Servicios de Hosting
====================

**Read the Docs:**
   - Sitio: https://readthedocs.org/
   - Docs: https://docs.readthedocs.io/

**GitHub Pages:**
   - Guide: https://pages.github.com/
   - Sphinx GH Pages: https://github.com/sphinx-doc/sphinx/tree/master/doc

**GitLab Pages:**
   - Docs: https://docs.gitlab.com/ee/user/project/pages/

**Netlify:**
   - Sitio: https://www.netlify.com/
   - Docs: https://docs.netlify.com/

Acciones CI/CD
==============

**GitHub Actions:**
   - Sphinx Build: https://github.com/marketplace/actions/sphinx-build

**GitLab CI:**
   - Example: https://gitlab.com/pages/sphinx

**Travis CI:**
   - Docs: https://docs.travis-ci.com/




Comunidad y Soporte
===================

Foros y Discusión
=================

**Stack Overflow:**
   - Sphinx: https://stackoverflow.com/questions/tagged/python-sphinx
   - RST: https://stackoverflow.com/questions/tagged/restructuredtext

**Reddit:**
   - r/Python: https://www.reddit.com/r/Python/
   - r/documentationdesign: https://www.reddit.com/r/documentationdesign/

**Sphinx Discord:**
   https://discord.gg/sphinx-doc

Listas de Correo
================

**Sphinx Users:**
   https://groups.google.com/g/sphinx-users

**Docutils Users:**
   https://sourceforge.net/p/docutils/mailman/

Blogs y Artículos
=================

**Real Python:**
   https://realpython.com/search?q=sphinx

**Full Stack Python:**
   https://www.fullstackpython.com/blog.html

**Python Weekly:**
   https://www.pythonweekly.com/




Libros Recomendados
===================

Documentación Técnica
=====================

**"Docs for Developers"** - Jared Bhatti et al.
   Apress, 2021

**"Technical Writing for Dummies"** - Sheryl Lindsell-Roberts
   Wiley, 2001

**"The Insider's Guide to Technical Writing"** - Krista Van Laan
   XML Press, 2012

Python y Sphinx
===============

**"Python Documentation with Sphinx"** - Online Courses
   Various platforms

**Pro Git** - Scott Chacon
   Apress (Free online: https://git-scm.com/book)

LaTeX
=====

**"The LaTeX Companion"** - Frank Mittelbach et al.
   Addison-Wesley, 2004

**"LaTeX Beginner's Guide"** - Stefan Kottwitz
   Packt, 2011




Canales de YouTube
==================

Tutoriales en Video
===================

**Corey Schafer:**
   https://www.youtube.com/c/Coreyms
   - Python tutorials, Git

**Tech With Tim:**
   https://www.youtube.com/c/TechWithTim
   - Python, documentation

**freeCodeCamp:**
   https://www.youtube.com/c/Freecodecamp
   - Full courses en documentación




Awesome Lists
=============

**Awesome Sphinx:**
   https://github.com/yoloseem/awesome-sphinxdoc

**Awesome Documentation:**
   https://github.com/PharkMillups/beautiful-docs

**Awesome Python:**
   https://github.com/vinta/awesome-python

**Awesome LaTeX:**
   https://github.com/egeerardyn/awesome-LaTeX




Herramientas de Escritura
=========================

Editores de Texto
=================

**VS Code:**
   - Sitio: https://code.visualstudio.com/
   - Extensión RST: https://marketplace.visualstudio.com/items?itemName=lextudio.restructuredtext

**Sublime Text:**
   - Sitio: https://www.sublimetext.com/
   - Package RST: https://packagecontrol.io/packages/Restructured%20Text%20%28RST%29%20Snippets

**Vim:**
   - Plugin RST: https://github.com/marshallward/vim-restructuredtext

**Emacs:**
   - rst-mode: Built-in

Correctores
===========

**LanguageTool:**
   https://languagetool.org/

**Grammarly:**
   https://www.grammarly.com/

**Hemingway Editor:**
   https://hemingwayapp.com/

Gestión de Referencias
======================

**Zotero:**
   https://www.zotero.org/

**Mendeley:**
   https://www.mendeley.com/

**BibTeX:**
   http://www.bibtex.org/




Accesibilidad
=============

**Web Content Accessibility Guidelines (WCAG):**
   https://www.w3.org/WAI/WCAG21/quickref/

**Sphinx Accessibility:**
   https://www.sphinx-doc.org/en/master/usage/builders/index.html#accessibility

**Color Contrast Checker:**
   https://webaim.org/resources/contrastchecker/




SEO y Analytics
===============

**Google Search Console:**
   https://search.google.com/search-console

**Moz - SEO Learning Center:**
   https://moz.com/learn/seo

**Google Analytics:**
   https://analytics.google.com/




.. seealso::
   
   * :doc:`cheatsheets/cheatsheet_rst` - Cheatsheet RST
   * :doc:`comandos_utiles` - Comandos útiles
   * :doc:`../07_guias_uso/tutorial_completo` - Tutorial ADT

.. note::
   Esta lista se actualiza periódicamente. Sugerencias de recursos adicionales son bienvenidas.

.. warning::
   Siempre verifica la vigencia de los enlaces, ya que algunos sitios pueden cambiar de URL o desaparecer.

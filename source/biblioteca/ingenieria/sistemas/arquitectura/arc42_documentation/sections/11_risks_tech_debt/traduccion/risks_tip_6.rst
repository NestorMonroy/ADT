...
.. code-block:: python
   ...
   # ❌ RIESGO: Cyclomatic Complexity = 15
   ...
   # ✅ MEJOR: Complexity = 2 (Strategy pattern)
   processor = OrderProcessorFactory.get(order)
   processor.process()

**2. Acoplamiento Alto**

* ⚠️ **Módulo depende de 10+ otros módulos**
...
* ⚠️ **Mismo código copy-pasted en múltiples lugares**
...
.. code-block:: text
   ...
   ❌ Tecnología obsoleta (PHP 5.3, Python 2.7)
   ❌ Sin documentación
   ❌ Un solo desarrollador conoce el código
   ❌ "Nadie se atreve a tocarlo"
   ❌ Dependencias sin actualizar hace años
...
.. code-block:: text
   ...
   Análisis Estático:
   ├─ LOC: 2,500 (⚠️ considerar split)
   ├─ Cyclomatic Complexity: Avg 8, Max 45 (🔴 CRÍTICO)
   ├─ Coupling: Depende de 15 módulos (⚠️ ALTO)
   ├─ Code Duplication: 12% (⚠️ ALTO)
   └─ Security Issues: 3 SQL injection risks (🔴 CRÍTICO)
   
   Análisis Dinámico:
   ├─ Test Coverage: 35% (🔴 BAJO)
   ├─ Performance: 200ms avg (✅ OK)
   ├─ Memory: No leaks detected (✅ OK)
